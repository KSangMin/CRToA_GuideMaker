...
- 아래에 정의된 서브 에이전트들의 역할을 명확히 구분하여 시스템에 내장된 서브에이전트 호출 툴(예: `invoke_subagent` tool)을 사용하여 작업을 분배(Delegate)하고 협업을 지휘하십시오. 
...
- 오케스트레이터는 유저에게 호출 프롬프트를 텍스트로 보여주고 종료하는 대신, 즉각 서브에이전트 호출 툴(`invoke_subagent`)을 사용해 백그라운드에서 `@planner`를 실행시키고 해당 태스크의 요구사항을 분석하여 `.ai_context/cur_task.md`를 작성하도록 자동 지시하십시오.